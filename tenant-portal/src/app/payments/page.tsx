...
// Lazy-load crypto component (heavy wagmi/viem deps, only needed if user selects USDT)
const CryptoPaymentFlow = dynamic(() => import('@/components/CryptoPaymentFlow'), { ssr: false });
import {
  CreditCard,
  Download,
  Filter,
...
function PaymentsContent() {
  ...
  useEffect(() => {
    ...
    // Show success toast when redirected from Paystack callback
    ...
    Promise.all([
      ...
      // Pre-fill amount: prefer next due amount, fall back to monthly rent
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {utilityCharges.map(charge => {
                  const icon = { electricity: Zap, water: Droplets, gas: Flame, internet: Wifi, waste: Trash2, security: Shield }[charge.utility_type] || Zap;
                  const Icon = icon;
                  return (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {autoPayEnabled && (
              <div className="space-y-4 animate-slide-down">
                ...
                <button
                  onClick={() => addToast('success', 'Auto-Pay Updated', 'Your auto-pay settings have been saved.')}
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl text-lg font-semibold focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                    placeholder="0.00"
                  />
                </div>
                ...
                {/* Fee Breakdown (hidden for USDT — CryptoPaymentFlow handles it) */}
                ...
                {/* Pay Button (hidden for USDT — CryptoPaymentFlow has its own) */}
                {paymentMethod !== 'usdt' && (
                <button
                  onClick={handleInitiatePayment}
                  disabled={!paymentAmount || parseFloat(paymentAmount) <= 0 || feeLoading}
            ...
            {paymentStep === 'failed' && (
              <div className="p-12 text-center">
                ...
                <div className="flex gap-3 mt-6 justify-center">
                  <button onClick={resetPaymentModal} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200">
                    Cancel
                  </button>
                  <button onClick={() => setPaymentStep('form')} className="px-5 py-2.5 bg-cyan-600 text-white rounded-xl font-medium hover:bg-cyan-700">
                    Try Again
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setDisputeChargeId(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <textarea
              ...
              placeholder="e.g. I was not occupying the unit during this period..."
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 resize-none"
              rows={3}
            />
