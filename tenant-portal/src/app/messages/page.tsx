...
function MessagesContent() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      loadConversations(); // refresh last message previews
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingConvos ? (
                ...
                <button
                  ...
                  className={`w-full px-4 py-3 flex items-start gap-3 text-left hover:bg-gray-50 transition-colors border-b border-gray-50 ${
                    activeConversation === convo.id ? 'bg-cyan-50/50' : ''
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {convo.lastMessageAt && (
                        <p className="text-[10px] text-gray-400 flex-shrink-0 ml-2">{formatMessageTime(convo.lastMessageAt)}</p>
                      )}
                    </div>
                    <p className="text-xs text-gray-500 truncate mt-0.5">{convo.lastMessage || 'No messages yet'}</p>
                  </div>
                  {(convo.unreadCount || 0) > 0 && (
                    <span className="w-5 h-5 bg-cyan-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center flex-shrink-0">
                      {convo.unreadCount}
                    </span>
                  )}
                </button>
