...
/** WalletConnect project ID — set in env or use a placeholder for dev */
const walletConnectProjectId = process.env.NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID || '';

/**
 * Wagmi config — supports Trust Wallet (injected) and WalletConnect.
 * Chains: Polygon mainnet + Amoy testnet.
 */
...
/**
 * USDT contract address (used for approve + allowance checks).
 * Defaults to Polygon mainnet USDT; override via env for testnet (MockUSDT on Amoy).
 */
export const USDT_ADDRESS = (process.env.NEXT_PUBLIC_USDT_ADDRESS || '0xc2132D05D31c914a87C6611C10748AEb04B58e8F') as `0x${string}`;

/**
 * PROPMETRIK contract address (from env).
 */
export const PROPMETRIK_CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROPMETRIK_CONTRACT as `0x${string}` | undefined;

/**
 * ERC-20 ABI (approve + allowance only — we don't need the full ABI).
 */
...
/**
 * PROPMETRIKPayments ABI (processPayment only — the frontend only calls this one function).
 */
