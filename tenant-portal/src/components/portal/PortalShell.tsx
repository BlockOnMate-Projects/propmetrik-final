...
export default function PortalShell({ children, title, subtitle }: PortalShellProps) {
  ...
  // Close dropdowns on click outside (ref-based for React 18 compatibility)
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button onClick={() => router.replace('/login')} className="text-cyan-600 hover:underline font-medium">
            Return to login
          </button>
        </div>
      </div>
          ...
          <div className="border-t border-gray-100 p-3 space-y-1">
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                pathname === '/settings' ? 'bg-cyan-50 text-cyan-700' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              ...
              {notificationsOpen && (
                <div className="absolute right-0 top-12 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 animate-slide-down overflow-hidden">
                  ...
                  <div className="px-4 py-2.5 border-t border-gray-100 text-center">
                    <Link href="/settings" className="text-xs text-cyan-600 hover:underline font-medium" onClick={() => setNotificationsOpen(false)}>
            ...
            <div className="relative ml-2" ref={profileMenuRef}>
              ...
              {profileMenuOpen && (
                <div className="absolute right-0 top-12 w-56 bg-white rounded-xl shadow-xl border border-gray-200 z-50 animate-slide-down overflow-hidden">
                  ...
                  <div className="p-1.5">
                    <Link href="/profile" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setProfileMenuOpen(false)}>
                      <User className="w-4 h-4" /> My Profile
                    </Link>
                    <Link href="/settings" className="flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setProfileMenuOpen(false)}>
          ...
          <div className="flex items-center justify-around h-16 max-w-md mx-auto">
            ...
            <div className="relative" ref={moreMenuRef}>
              <button
                ...
                className={`flex flex-col items-center justify-center gap-0.5 w-16 py-1 ${
                  ['/messages', '/documents', '/profile', '/settings'].some(p => pathname.startsWith(p))
                    ? 'text-cyan-600'
                    : 'text-gray-400'
                }`}
              >
              ...
              {moreMenuOpen && (
                <div className="absolute bottom-16 right-0 w-48 bg-white rounded-xl shadow-xl border border-gray-200 z-50 animate-slide-up overflow-hidden">
                  <div className="p-1.5">
                    <Link href="/messages" className="flex items-center gap-2.5 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setMoreMenuOpen(false)}>
                      <MessageSquare className="w-4 h-4" /> Messages
                    </Link>
                    <Link href="/documents" className="flex items-center gap-2.5 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setMoreMenuOpen(false)}>
                      <FileText className="w-4 h-4" /> Documents
                    </Link>
                    <Link href="/profile" className="flex items-center gap-2.5 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setMoreMenuOpen(false)}>
                      <User className="w-4 h-4" /> My Profile
                    </Link>
                    <Link href="/settings" className="flex items-center gap-2.5 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setMoreMenuOpen(false)}>
                      <Settings className="w-4 h-4" /> Settings
                    </Link>
                  </div>
                  <div className="p-1.5 border-t border-gray-100">
