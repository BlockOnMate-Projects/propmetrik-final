...
/**
 * Generate an image from text - used for date, name, initials, etc.
 * This ensures ALL field types are stored as images for consistent PDF embedding
 */
function generateTextAsImage(
  text: string, 
  options: { 
    fontSize?: number; 
    fontFamily?: string; 
  ...
  const scale = 2; // High DPI for crisp rendering
...
export default function FieldPlacement({ 
  ...
  const handlePageDrop = (e: React.DragEvent, pageNum: number, pageElement: HTMLDivElement) => {
    ...
    // ALL field types generate images for consistent PDF embedding
    if (isSelfSigning) {
      if (newField.type === 'signature') {
        // Check for saved signature first
        const savedSig = getSavedSignature();
        if (savedSig) {
          storeLocalValue(newField.id, undefined, savedSig);
          onFieldSigned?.(newField.id, savedSig);
        } else {
          setFieldToSign(newField);
          setShowSignatureModal(true);
        }
      } else if (newField.type === 'initial') {
        const initials = generateInitials(currentUser?.name || '');
        const imageData = generateTextAsImage(initials, { fontSize: 18, isInitials: true, width: 80, height: 40 });
      ...
      } else if (newField.type === 'name' && currentUser) {
        const imageData = generateTextAsImage(currentUser.name, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(newField.id, currentUser.name, signatureData);
        onFieldSigned?.(newField.id, signatureData, currentUser.name);
      } else if (newField.type === 'email' && currentUser) {
        const imageData = generateTextAsImage(currentUser.email, { fontSize: 14, width: 250, height: 35 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(newField.id, currentUser.email, signatureData);
        onFieldSigned?.(newField.id, signatureData, currentUser.email);
      } else if (newField.type === 'checkbox') {
        const imageData = generateTextAsImage('', { isCheckbox: true, checked: true, width: 30, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(newField.id, 'checked', signatureData);
        onFieldSigned?.(newField.id, signatureData, 'checked');
      } else if (newField.type === 'company') {
        const companyName = currentUser?.name ? `${currentUser.name}'s Company` : 'Company';
        const imageData = generateTextAsImage(companyName, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(newField.id, companyName, signatureData);
        onFieldSigned?.(newField.id, signatureData, companyName);
      } else if (newField.type === 'title') {
        const titleText = 'Authorized Signatory';
        const imageData = generateTextAsImage(titleText, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(newField.id, titleText, signatureData);
        onFieldSigned?.(newField.id, signatureData, titleText);
      }
    } else {
  ...
  const handleFieldClick = (e: React.MouseEvent, field: PlacedField) => {
    ...
    if (isSelfSigning && !signedFields.has(field.id)) {
      if (field.type === 'signature') {
        ...
        // Generate initials as IMAGE
        const initials = generateInitials(currentUser?.name || '');
        const imageData = generateTextAsImage(initials, { 
          fontSize: 18, 
          isInitials: true,
          width: 80,
          height: 40
        ...
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, initials, signatureData);
        ...
        // Generate name as IMAGE
        const imageData = generateTextAsImage(currentUser.name, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, currentUser.name, signatureData);
        onFieldSigned?.(field.id, signatureData, currentUser.name);
      } else if (field.type === 'email' && currentUser) {
        // Generate email as IMAGE
        const imageData = generateTextAsImage(currentUser.email, { fontSize: 14, width: 250, height: 35 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, currentUser.email, signatureData);
        onFieldSigned?.(field.id, signatureData, currentUser.email);
      } else if (field.type === 'checkbox') {
        // Generate checkbox as IMAGE
        const imageData = generateTextAsImage('', { isCheckbox: true, checked: true, width: 30, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, 'checked', signatureData);
        onFieldSigned?.(field.id, signatureData, 'checked');
      } else if (field.type === 'company') {
        // For company, use a placeholder or prompt user
        const companyName = currentUser?.name ? `${currentUser.name}'s Company` : 'Company';
        const imageData = generateTextAsImage(companyName, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, companyName, signatureData);
        onFieldSigned?.(field.id, signatureData, companyName);
      } else if (field.type === 'title') {
        // For title, use a placeholder
        const titleText = 'Authorized Signatory';
        const imageData = generateTextAsImage(titleText, { fontSize: 14, width: 180, height: 30 });
        const signatureData: SignatureData = { type: 'typed', data: imageData };
        storeLocalValue(field.id, titleText, signatureData);
        onFieldSigned?.(field.id, signatureData, titleText);
      }
    } else {
      setSelectedField(field);
    }
