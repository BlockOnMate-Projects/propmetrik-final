...
import { getSignerInfo, signDocumentPublic, declineSignature, getDocumentForSigning } from '../api';
...
function SignaturePage() {
  ...
  const [signatureFields, setSignatureFields] = useState<SignatureFieldInfo[]>([]);
  ...
  const loadSignerInfo = async () => {
    ...
    try {
      ...
      const docResponse = await getDocumentForSigning(accessToken);
      setSignatureFields(docResponse.data?.signature_fields || []);
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      const placement = signatureFields[0];
      await signDocumentPublic(accessToken, signatureData, 'drawn', placement);
