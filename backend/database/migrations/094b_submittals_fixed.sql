-- Migration: 094b_submittals_fixed.sql
-- Description: Submittals management system - Fixed without contractors FK
-- Phase 3A Week 2
...
DROP TABLE IF EXISTS submittal_package_items CASCADE;
DROP TABLE IF EXISTS submittal_packages CASCADE;
DROP TABLE IF EXISTS submittal_history CASCADE;
DROP TABLE IF EXISTS submittal_reviews CASCADE;
DROP TABLE IF EXISTS submittal_distribution CASCADE;
DROP TABLE IF EXISTS submittals CASCADE;

-- Main submittals table (without FK to contractors)
CREATE TABLE submittals (
    ...
    -- Assignments (using UUIDs without FK for flexibility)
    submitted_by UUID,
...
CREATE INDEX idx_submittals_contractor ON submittals(contractor_id);
CREATE INDEX idx_submittals_reviewer ON submittals(assigned_reviewer);
CREATE INDEX idx_submittals_submitted_date ON submittals(submitted_date);
CREATE INDEX idx_submittals_required_date ON submittals(required_date);
CREATE INDEX idx_submittals_spec_section ON submittals(spec_section);
CREATE INDEX idx_submittal_reviews_submittal ON submittal_reviews(submittal_id);
CREATE INDEX idx_submittal_history_submittal ON submittal_history(submittal_id);
...
CREATE OR REPLACE VIEW submittal_details AS
SELECT 
    s.*,
    dp.name as project_name,
    CASE 
...
-- Success message
DO $$ BEGIN RAISE NOTICE 'Migration 094b_submittals_fixed.sql completed successfully'; END $$;
