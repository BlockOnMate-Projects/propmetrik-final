-- Migration 093c: Final fix for Change Orders Tables
-- Fix enum values and user column names
...
DROP TRIGGER IF EXISTS trigger_co_completion_date ON change_orders;

DROP FUNCTION IF EXISTS generate_co_number() CASCADE;
DROP FUNCTION IF EXISTS update_co_timestamp() CASCADE;
DROP FUNCTION IF EXISTS update_co_totals() CASCADE;
DROP FUNCTION IF EXISTS recalculate_co_totals() CASCADE;
DROP FUNCTION IF EXISTS calculate_co_completion_date() CASCADE;
...
-- =====================================================
-- CHANGE ORDERS TABLE
-- Using existing change_order_type enum: additive, deductive, no_cost, time_extension
-- =====================================================
...
-- RFI Summary View (using first_name || last_name for full name)
...
SELECT 
  ...
  CONCAT(responder.first_name, ' ', responder.last_name) AS answered_by_name,
...
SELECT 
  ...
  CONCAT(executor.first_name, ' ', executor.last_name) AS executed_by_name,
  ph.name AS phase_name,
  (
    SELECT COUNT(*) 
