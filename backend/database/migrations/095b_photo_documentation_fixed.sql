-- Migration 095b: Photo Documentation System (Fixed - No FK Constraints)
...
-- Main site_photos table (no FK constraints for flexibility)
CREATE TABLE IF NOT EXISTS site_photos (
    ...
    -- Relationships (stored as UUIDs without FK)
    related_rfi_id UUID,
...
-- Notification logs for WhatsApp (if not exists)
...
DROP FUNCTION IF EXISTS soft_delete_photo(UUID);
CREATE FUNCTION soft_delete_photo(p_photo_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE site_photos SET deleted_at = NOW() WHERE id = p_photo_id AND deleted_at IS NULL;
    RETURN FOUND;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    LOOP
        ...
        SELECT EXISTS(SELECT 1 FROM photo_share_links WHERE share_token = token) INTO token_exists;
...
COMMENT ON TABLE notification_logs IS 'Log of all WhatsApp and other notifications sent';
