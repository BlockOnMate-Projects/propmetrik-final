...
from models import (
    ...
    SignatureRequestStatus, AuditLog, User, SignatureField
)
...
from api.documents import get_or_create_user
import uuid
import os
...
async def sign_document(
    ...
    if all_signed:
        if signature_request.security_hash:
            signature_request.status = SignatureRequestStatus.COMPLETED
            signature_request.completed_at = signature_request.completed_at or datetime.utcnow()
            db.commit()
            return signature
        completion_ts = datetime.utcnow().replace(microsecond=0).isoformat() + "Z"

        # Build signature events (deterministic)
        signature_events = []
        signatures = db.query(Signature).filter(Signature.signature_request_id == signature_request.id).all()
        for sig in signatures:
            signer = db.query(Signer).filter(Signer.id == sig.signer_id).first()
            # Use any available signature field for coordinates
            field = db.query(SignatureField).filter(SignatureField.signer_id == sig.signer_id).order_by(SignatureField.created_at.desc()).first()
            if not field:
                continue
            signer_user = db.query(User).filter(User.email == signer.email).first() if signer else None
            signer_pmt_id = signer.signer_pmt_id if signer else (signer_user.signer_id if signer_user else None)
            signature_events.append({
                ...
                "width": field.width,
                "height": field.height,
                "signed_at": sig.signed_at.replace(microsecond=0).isoformat() + "Z",
                "signer_id": signer_pmt_id or "PMT-UNKNOWN",
        ...
        if not document or not document.file_path:
            raise HTTPException(status_code=500, detail="Document file not found for certification")

        pdf_path = os.path.join(settings.UPLOAD_DIR, document.file_path)
        if not os.path.exists(pdf_path):
            raise HTTPException(status_code=500, detail="Document file missing on disk")

        with open(pdf_path, "rb") as f:
            original_pdf_bytes = f.read()

        signed_pdf_bytes = embed_signatures_into_pdf(original_pdf_bytes, signature_events)
        ...
        signer_list = [
            ...
            for e in sorted(signature_events, key=lambda e: (e["signed_at"], e["signer_id"]))
        ]
        ...
        signed_filename = f"signed_{document.id}_{uuid.uuid4().hex}.pdf"
        signed_rel_path = os.path.join("signed", signed_filename)
        signed_abs_path = os.path.join(settings.UPLOAD_DIR, signed_rel_path)
        persist_pdf(signed_abs_path, final_pdf_bytes)

        document.signed_file_path = signed_rel_path
        signature_request.status = SignatureRequestStatus.COMPLETED
        signature_request.completed_at = datetime.utcnow()
        signature_request.security_hash = security_hash
        signature_request.security_hash_generated_at = datetime.utcnow()
        signature_request.certificate_file_path = signed_rel_path
        db.commit()
    ...
    log_signing_audit_event(
        ...
        event_data={
            ...
            "signature_type": sign_request.signature_type,
            "signer_pmt_id": signer.signer_pmt_id
        }
    )
...
async def get_document_for_signing(
    ...
    fields = db.query(SignatureField).filter(SignatureField.signer_id == signer.id).all()
