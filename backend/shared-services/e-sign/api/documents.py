...
def get_or_create_user(propmetrik_user_id: str, email: str, full_name: Optional[str], db: Session, organization_id: Optional[str] = None) -> User:
    ...
    # Try by propmetrik_user_id first
    ...
    if not user:
        ...
        if user and not user.propmetrik_user_id:
            # Update existing user with propmetrik_user_id
            user.propmetrik_user_id = propmetrik_user_id
            if organization_id:
                user.organization_id = organization_id
            if not user.signer_id:
                user.signer_id = f"PMT-{secrets.token_hex(6).upper()}"
            db.commit()
            db.refresh(user)
    if not user:
        user = User(
            ...
            signer_id=f"PMT-{secrets.token_hex(6).upper()}"
        )
        db.add(user)
        db.commit()
        db.refresh(user)
