...
class ComplianceReportService {
  ...
  async generateReport(input: GenerateReportInput): Promise<ComplianceReportResult> {
    ...
    // Generate PDF
    const pdfBuffer = await this.buildPDF(reportData, input);
  ...
  private async gatherReportData(input: GenerateReportInput): Promise<ComplianceReportData> {
    const [project, dashboard, authorities] = await Promise.all([
      projectService.getById(input.project_id),
    ...
    return {
      project: {
        id: project.id,
        name: project.name,
        location: `${project.sub_metro || ''} ${project.assembly || ''}, ${project.region || ''}`.trim(),
   ...
   */
  private async buildPDF(data: ComplianceReportData, input: GenerateReportInput): Promise<Buffer> {
