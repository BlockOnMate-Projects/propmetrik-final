...
class PaymentPlanService {
  private static instance: PaymentPlanService;

  private constructor() {}

  public static getInstance(): PaymentPlanService {
    if (!PaymentPlanService.instance) {
      PaymentPlanService.instance = new PaymentPlanService();
    }
    return PaymentPlanService.instance;
  }
  ...
  async create(input: CreatePaymentPlanInput): Promise<BuyerPaymentPlan> {
    ...
    try {
      ...
      logger.info({ planId: result.rows[0].id }, 'Payment plan created');
      ...
      logger.error({ error, input }, 'Failed to create payment plan');
      throw error;
    } finally {
      client.release();
    }
  ...
  private mapRow(row: any): Omit<BuyerPaymentPlan, 'payments' | 'buyer_name' | 'buyer_email' | 'buyer_phone'> {
...
export default PaymentPlanService.getInstance();
