...
class DrawService {
  ...
  async create(input: CreateDrawInput): Promise<DrawRequest> {
    ...
    try {
      ...
      logger.info({ drawId: result.rows[0].id }, 'Draw request created');
      
      return this.mapRow(result.rows[0]);
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error({ error, input }, 'Failed to create draw request');
      throw error;
    } finally {
      client.release();
    }
  ...
  private mapRow(row: any): DrawRequest {
    return {
      id: row.id,
...
export default DrawService.getInstance();
