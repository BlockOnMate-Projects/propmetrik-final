...
async function processInspectionReminder(
  ...
  await createNotification({
    ...
    message: `Inspection scheduled for ${permit.permit_name} on ${new Date(scheduledDate).toLocaleDateString()}`,
    type: 'inspection_reminder',
...
async function processRenewalAlert(
  ...
  await createNotification({
    ...
    message: `${permit.permit_name} for project "${project.name}" expires on ${new Date(permit.expiration_date!).toLocaleDateString()}. Please initiate renewal process.`,
  ...
  for (const doc of expiringDocuments) {
    ...
    if ([30, 14, 7, 1].includes(daysUntilExpiry)) {
      await createNotification({
        ...
        message: `"${doc.name}" expires on ${expirationDate.toLocaleDateString()}`,
        type: 'document_expiration',
