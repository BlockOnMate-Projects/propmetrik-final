...
function UpgradeModal({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md mx-4 border border-zinc-700 bg-zinc-900 shadow-2xl shadow-amber-500/10">
        {/* Header */}
        <div className="px-6 py-4 border-b border-zinc-800 bg-zinc-800/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-mono text-sm text-white tracking-wide">
        ...
        <div className="px-6 py-5">
          <div className="mb-4">
            ...
            <p className="font-mono text-[11px] text-zinc-400 leading-relaxed">{gate.description}</p>
          </div>

          {!isRoleRestricted && (
            <>
              {/* Current vs Required */}
              <div className="flex items-center gap-3 mb-5 p-3 bg-zinc-800/50 border border-zinc-700">
                <div className="flex-1 text-center">
                  <div className="font-mono text-[9px] text-zinc-500 mb-1">CURRENT PLAN</div>
                  <div className="font-mono text-xs text-white">{getTierDisplayName(currentTier)}</div>
                </div>
                <svg className="w-4 h-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
                <div className="flex-1 text-center">
                  <div className="font-mono text-[9px] text-zinc-500 mb-1">REQUIRED</div>
                  <div className="font-mono text-xs text-amber-400">{requiredTier}</div>
                </div>
              </div>

              {/* Benefits tease */}
              <div className="mb-5 space-y-2">
                <div className="font-mono text-[10px] text-zinc-500 mb-2">INCLUDED WITH {requiredTier.toUpperCase()}:</div>
                {getTierBenefits(gate.minTier).map((benefit, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[10px] text-zinc-300">{benefit}</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="px-6 py-4 border-t border-zinc-800 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2 font-mono text-xs text-zinc-400 border border-zinc-700 hover:bg-zinc-800 transition-colors"
          >
            CLOSE
          </button>
          {!isRoleRestricted && (
...
export function LockedBadge({ featureKey, className }: { featureKey: string; className?: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 font-mono text-[8px]', className)}>
