...
export function ProjectHeader({ project, isLoading, projectManagerName }: ProjectHeaderProps) {
  ...
  return (
    <div className="mb-8 space-y-4">
      ...
      {(project.land_tenure_type || project.traditional_authority_id) && (
         <div className="flex items-center gap-4 py-2 px-3 bg-zinc-900/40 border border-zinc-800/60 rounded-md">
            {project.land_tenure_type && (
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono text-zinc-500 uppercase">Land Tenure</span>
                    <span className="text-xs text-zinc-300 font-medium capitalize">{project.land_tenure_type.replace(/_/g, ' ')}</span>
                </div>
            )}
