  ...
  if (isLoading) {
    return (
      <div className="border border-zinc-800 bg-zinc-900/50 p-6 text-center">
        ...
        <p className="font-mono text-xs text-zinc-500 mt-2">Loading frameworks...</p>
      </div>
    )
  }
  ...
  if (frameworks.length === 0) {
    return (
      <div className="border border-zinc-800 bg-zinc-900/50 p-6 text-center">
        ...
        <h4 className="font-mono text-sm text-white mb-1">No Frameworks Available</h4>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-xs text-zinc-400">Milestone Framework</span>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          <div className={cn(
            ...
            !selectedFrameworkId ? "bg-amber-900/50 text-amber-400" : "bg-zinc-800 text-zinc-500"
          )}>
            <Unlock className="h-5 w-5" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h4 className="font-mono text-sm text-white">No Framework</h4>
              {!selectedFrameworkId && (
          ...
          return (
            <div key={framework.id} className="relative">
              ...
              >
                <div className={cn(
                  "flex-shrink-0 w-10 h-10 flex items-center justify-center",
                  isSelected ? "bg-amber-900/50 text-amber-400" : "bg-zinc-800 text-zinc-500"
                )}>
                  <Shield className="h-5 w-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className="font-mono text-sm text-white">{framework.name}</h4>
                    {isSelected && (
                      <CheckCircle2 className="h-4 w-4 text-amber-500" />
                    )}
                    {framework.is_locked && (
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <Badge className="font-mono text-[10px] bg-zinc-800 text-zinc-400">
                      v{framework.version}
                    </Badge>
                    {framework.region && (
                      <Badge className="font-mono text-[10px] bg-zinc-800 text-zinc-400">
              ...
              {showDetails && (
                <button
                  ...
                  className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-amber-500 transition-colors"
                >
      ...
      {selectedFramework && showDetails && (
        <div className="border border-amber-800/50 bg-amber-900/20 p-4">
          <div className="flex items-center gap-2 mb-3">
            <Shield className="h-4 w-4 text-amber-500" />
            <span className="font-mono text-xs text-amber-400">Selected Framework</span>
          </div>
          <h4 className="font-mono text-sm text-white mb-1">{selectedFramework.name}</h4>
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="bg-zinc-900 border-zinc-700 max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {/* Framework Info */}
            <div className="grid grid-cols-3 gap-4 bg-zinc-800/50 border border-zinc-700 p-4">
              <div>
                <span className="font-mono text-[10px] text-zinc-500">Project Type</span>
                <p className="font-mono text-sm text-white capitalize">
                  {previewFramework?.project_type?.replace('_', ' ')}
                </p>
              </div>
              <div>
                <span className="font-mono text-[10px] text-zinc-500">Version</span>
            ...
            {/* Phases */}
            <div>
              <h5 className="font-mono text-xs text-zinc-400 mb-3 flex items-center gap-2">
              ...
              {isLoadingPhases ? (
                ...
                <div className="text-center py-8 border border-zinc-800 bg-zinc-800/30">
                  <Layers className="h-8 w-8 text-zinc-700 mx-auto mb-2" />
                  <p className="font-mono text-xs text-zinc-500">No phases defined</p>
                </div>
              ) : (
                    ...
                    .map((phase, index) => (
                      <div key={phase.id} className="border border-zinc-800 bg-zinc-900/50">
                        ...
                        {phase.milestone_templates && phase.milestone_templates.length > 0 && (
                          <div className="border-t border-zinc-800 p-4 pl-16">
                            <div className="space-y-2">
                              {phase.milestone_templates.map((template) => (
                                <div key={template.id} className="flex items-center gap-3">
                                  <Flag className={cn(
                                    "h-3 w-3",
                                    template.is_required ? "text-red-400" : "text-zinc-500"
                                  )} />
                                  ...
                                  <div className="flex items-center gap-2">
                                    {template.requires_client_approval && (
                                      <Badge className="font-mono text-[8px] bg-amber-900/30 text-amber-400">
                                        Client Approval
                                      </Badge>
                                    )}
                                    {template.requires_inspection && (
                                      <Badge className="font-mono text-[8px] bg-blue-900/30 text-blue-400">
                                        Inspection
                                      </Badge>
                                    )}
                                  </div>
