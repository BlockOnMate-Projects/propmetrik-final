        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="font-mono text-sm text-muted-foreground">Loading invoice...</p>
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center p-4">
                <div className="bg-card border border-border rounded-xl p-8 max-w-md w-full text-center">
                    ...
                    <h1 className="font-mono text-lg text-foreground font-bold mb-2">Invoice Not Found</h1>
                    <p className="font-mono text-sm text-muted-foreground">{error || 'This invoice link may be invalid or expired.'}</p>
                </div>
            </div>
        )
    }
                    ...
                    <div className="p-8 text-center">
                        ...
                        <div className="bg-muted/50 border border-border rounded-lg p-4 mb-6">
                            <div className="flex justify-between font-mono text-sm mb-2">
                                ...
                                <span className="text-green-600 dark:text-green-400 font-bold">{formatCurrency(invoice.totalAmount, invoice.currency)}</span>
                            </div>
                            ...
                            {invoice.paidAt && (
                                <div className="flex justify-between font-mono text-xs mt-1">
                                    ...
                                    <span className="text-muted-foreground">{new Date(invoice.paidAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                                </div>
                            )}
                        </div>
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center p-4">
                <div className="bg-card border border-border rounded-xl p-8 max-w-md w-full text-center">
                    ...
                    <h1 className="font-mono text-lg text-foreground font-bold mb-2">Invoice Cancelled</h1>
                    <p className="font-mono text-sm text-muted-foreground">This invoice has been cancelled and is no longer payable.</p>
                </div>
            </div>
        )
    }
                ...
                <div className="bg-card border border-border rounded-xl max-w-2xl w-full overflow-hidden">
                    ...
                    <div className="bg-background px-8 py-6">
                        <div className="flex items-start justify-between">
                            ...
                            <div className="text-right">
                                <div className="font-mono text-muted-foreground text-xs">INVOICE</div>
                                <div className="font-mono text-amber-500 text-sm font-bold">{invoice.invoiceNumber}</div>
                                {isOverdue && (
                                    <div className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 bg-red-500/10 border border-red-500/20 rounded font-mono text-[9px] text-red-600 dark:text-red-400 font-bold">
                    ...
                    <div className="px-8 py-5 border-b border-border">
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <div className="font-mono text-[9px] text-muted-foreground uppercase tracking-wider mb-1">Bill To</div>
                                <div className="font-mono text-sm text-foreground font-medium">{invoice.clientName}</div>
                                {invoice.clientEmail && <div className="font-mono text-xs text-muted-foreground mt-0.5">{invoice.clientEmail}</div>}
                            </div>
                            <div className="text-right space-y-1">
                                <div className="flex justify-end gap-6">
                                    <div>
                                        <div className="font-mono text-[9px] text-muted-foreground uppercase tracking-wider">Date</div>
                                    ...
                                    {dueDate && (
                                        <div>
                                            <div className="font-mono text-[9px] text-muted-foreground uppercase tracking-wider">Due</div>
                                            <div className={`font-mono text-xs ${isOverdue ? 'text-red-600 dark:text-red-400 font-bold' : 'text-muted-foreground'}`}>{dueDate}</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-border">
                                        <th className="text-left py-1.5 font-mono text-[9px] text-muted-foreground uppercase tracking-wider">Description</th>
                                        <th className="text-center py-1.5 font-mono text-[9px] text-muted-foreground uppercase tracking-wider w-16">Qty</th>
                                        <th className="text-right py-1.5 font-mono text-[9px] text-muted-foreground uppercase tracking-wider w-28">Amount</th>
                                    </tr>
                                </thead>
                                <tbody className="font-mono text-xs">
                                    {invoice.lineItems.map((item, i) => (
                                        <tr key={i} className="border-b border-border/50">
                                            ...
                                            <td className="py-2 text-right text-muted-foreground">{formatCurrency(item.amount, invoice.currency)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="flex justify-end mt-3">
                            <div className="w-56 space-y-1">
                                <div className="flex justify-between font-mono text-xs">
                                    ...
                                    <span className="text-muted-foreground">{formatCurrency(invoice.subtotal, invoice.currency)}</span>
                                </div>
                                <div className="flex justify-between font-mono text-base font-bold border-t border-border pt-2 mt-1">
                                ...
                                {invoice.currency !== 'GHS' && invoice.paystackAmountGHS && invoice.fxRate > 1 && (
                                    <div className="font-mono text-[9px] text-muted-foreground text-right mt-1">
                    ...
                    <div className="px-8 py-6">
                        <h3 className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-4">Choose Payment Method</h3>
                            ...
                            {([
                                ...
                                >
                                    ...
                                    <method.icon className={`w-5 h-5 mb-2 ${
                                        paymentMethod === method.key
                                            ? method.key === 'crypto' ? 'text-purple-600 dark:text-purple-400' : 'text-amber-500'
                                            : 'text-muted-foreground'
                                    }`} />
                                    <div className="font-mono text-xs text-foreground font-medium">{method.label}</div>
                                    <div className="font-mono text-[9px] text-muted-foreground mt-0.5">{method.sublabel}</div>
                                </button>
                            ))}
                        </div>

                        {/* Payment Details */}
                        <div className="bg-muted/30 border border-border rounded-lg p-5 mb-5">
                            {paymentMethod === 'momo' && (
                                <div className="space-y-3">
                                    <div className="flex items-start gap-3">
                                        <Smartphone className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
                                        <div>
                                            <div className="font-mono text-sm text-foreground font-medium">Pay with Mobile Money</div>
                                    ...
                                    <div className="bg-card/50 border border-border/50 rounded p-3 flex items-center gap-2">
                                        <Shield className="w-3.5 h-3.5 text-green-500" />
                                        <span className="font-mono text-[10px] text-muted-foreground">Secured by Paystack &bull; 256-bit encryption</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {paymentMethod === 'card' && (
                                <div className="space-y-3">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <div className="font-mono text-sm text-foreground font-medium">Pay with Card</div>
                                            <p className="font-mono text-[10px] text-muted-foreground mt-1">
                                    ...
                                    <div className="bg-card/50 border border-border/50 rounded p-3 flex items-center gap-2">
                                        <Shield className="w-3.5 h-3.5 text-green-500" />
                                        <span className="font-mono text-[10px] text-muted-foreground">PCI DSS Compliant &bull; Secured by Paystack</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {paymentMethod === 'crypto' && (
                                <div className="space-y-3">
                                    <div className="flex items-start gap-3">
                                        <Wallet className="w-5 h-5 text-purple-600 dark:text-purple-400 shrink-0 mt-0.5" />
                                        <div>
                                            <div className="font-mono text-sm text-foreground font-medium">Pay with Crypto</div>
                                            <p className="font-mono text-[10px] text-muted-foreground mt-1">
                                    ...
                                    <div className="bg-card/50 border border-border/50 rounded p-3 flex items-center gap-2">
                                        ...
                                        <span className="font-mono text-[10px] text-muted-foreground">Powered by NOWPayments &bull; Auto-conversion</span>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center justify-between bg-background border border-border rounded-lg p-4">
                            <div>
                                <div className="font-mono text-[10px] text-muted-foreground uppercase tracking-wider">Amount to Pay</div>
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-3 font-mono text-sm font-bold rounded-lg transition-all ${
                                    paymentMethod === 'crypto'
                                        ? 'bg-purple-600 text-foreground hover:bg-purple-500'
                    ...
                    {/* Footer */}
                    <div className="px-8 py-4 bg-background/50 border-t border-border">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <FileText className="w-3.5 h-3.5 text-muted-foreground" />
                                <span className="font-mono text-[9px] text-muted-foreground">Invoice {invoice.invoiceNumber}</span>
                            </div>
