...
const STATUS_MAP: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
  draft: { label: 'Draft', cls: 'border-zinc-600 text-zinc-400', icon: <FileText className="h-3 w-3" /> },
  published: { label: 'Published', cls: 'border-blue-600 text-blue-400', icon: <Send className="h-3 w-3" /> },
  closed: { label: 'Closed', cls: 'border-yellow-600 text-yellow-400', icon: <Clock className="h-3 w-3" /> },
  under_evaluation: { label: 'Evaluating', cls: 'border-purple-600 text-purple-400', icon: <Eye className="h-3 w-3" /> },
  awarded: { label: 'Awarded', cls: 'border-green-600 text-green-400', icon: <Trophy className="h-3 w-3" /> },
  cancelled: { label: 'Cancelled', cls: 'border-red-600 text-red-400', icon: <XCircle className="h-3 w-3" /> },
  archived: { label: 'Archived', cls: 'border-zinc-700 text-zinc-500', icon: <FileText className="h-3 w-3" /> },
}

const SUB_STATUS: Record<string, { label: string; cls: string }> = {
  submitted: { label: 'Submitted', cls: 'border-blue-600 text-blue-400' },
  shortlisted: { label: 'Shortlisted', cls: 'border-amber-600 text-amber-400' },
  rejected: { label: 'Rejected', cls: 'border-red-600 text-red-400' },
  awarded: { label: 'Awarded', cls: 'border-green-600 text-green-400' },
}
...
export default function BidManagementPage() {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={() => setSelectedId(null)} className="text-zinc-400 hover:text-white gap-1">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-lg font-bold text-white">{detail.title}</h1>
              <Badge variant="outline" className={s.cls}>{s.label}</Badge>
            </div>
            <p className="text-xs font-mono text-zinc-500">{detail.project_name} · {detail.trade_category?.replace(/_/g, ' ')?.toUpperCase()}</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            {detail.status === 'draft' && (
              <>
                <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-300 text-xs" onClick={() => openEdit(detail)}>Edit</Button>
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white text-xs" onClick={() => publishMut.mutate(detail.id)}
            ...
            {detail.status === 'published' && (
              <Button size="sm" variant="outline" className="border-yellow-700 text-yellow-400 text-xs" onClick={() => closeMut.mutate(detail.id)}>
                Close Bidding
              </Button>
            )}
            {['published', 'closed', 'under_evaluation'].includes(detail.status) && submissions.filter(s => s.status === 'shortlisted').length > 0 && !awards.length && (
              <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs" onClick={() => setShowAwardDialog(true)}>
                <Trophy className="h-3 w-3 mr-1" /> Award Bid
              </Button>
            )}
            {detail.status === 'draft' && (
              <Button size="sm" variant="ghost" className="text-red-400 text-xs" onClick={() => { if (confirm('Delete this bid request?')) deleteMut.mutate(detail.id) }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
            ...
            <span className="text-white text-sm font-mono">{invitations.length}</span>
          </CardContent></Card>
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
            <span className="text-[10px] font-mono text-zinc-500 block">SUBMISSIONS</span>
            <span className="text-white text-sm font-mono">{submissions.length}</span>
          </CardContent></Card>
          {detail.estimated_budget && (
            <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
              <span className="text-[10px] font-mono text-zinc-500 block">EST. BUDGET</span>
              <span className="text-amber-400 text-sm font-mono">{detail.currency} {Number(detail.estimated_budget).toLocaleString()}</span>
            </CardContent></Card>
          )}
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
            <span className="text-[10px] font-mono text-zinc-500 block">QUESTIONS</span>
            <span className="text-white text-sm font-mono">{qa.length}</span>
          </CardContent></Card>
        </div>
        ...
        <Tabs defaultValue="invitations" className="space-y-4">
          <TabsList className="bg-zinc-900 border border-zinc-800">
            <TabsTrigger value="invitations" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 text-xs">
              <Users className="h-3 w-3 mr-1" /> Invitations ({invitations.length})
            </TabsTrigger>
            <TabsTrigger value="submissions" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 text-xs">
              <DollarSign className="h-3 w-3 mr-1" /> Submissions ({submissions.length})
            </TabsTrigger>
            <TabsTrigger value="qa" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 text-xs">
              <MessageSquare className="h-3 w-3 mr-1" /> Q&A ({qa.length})
            </TabsTrigger>
            <TabsTrigger value="attachments" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 text-xs">
              <Paperclip className="h-3 w-3 mr-1" /> Files ({detail.attachments?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="activity" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 text-xs">
          ...
          <TabsContent value="invitations" className="space-y-4">
            ...
            {invitations.length === 0 ? (
              <div className="text-center py-12 text-zinc-600 text-sm">No vendors invited yet</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-zinc-800">
                    <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">VENDOR</th>
                    <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">COMPANY</th>
                    <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">STATUS</th>
                    <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">VIEWED</th>
                    <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">SUBMITTED</th>
                    <th className="text-right py-2 px-3 text-[10px] font-mono text-zinc-500">ACTIONS</th>
                  </tr></thead>
                  <tbody>
                    {invitations.map(inv => (
                      <tr key={inv.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/50">
                        <td className="py-2 px-3">
                          <div className="text-white text-sm">{inv.vendor_name}</div>
                          <div className="text-zinc-500 text-xs">{inv.vendor_email}</div>
                        </td>
                        <td className="py-2 px-3 text-zinc-400 text-sm">{inv.vendor_company}</td>
                        <td className="py-2 px-3">
                          <Badge variant="outline" className={
                            inv.status === 'pending' ? 'border-zinc-600 text-zinc-400' :
                            inv.status === 'viewed' ? 'border-blue-600 text-blue-400' :
                            inv.status === 'declined' ? 'border-red-600 text-red-400' :
                            inv.status === 'submitted' ? 'border-green-600 text-green-400' :
                        ...
                        <td className="py-2 px-3 text-right">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500"><MoreVertical className="h-3 w-3" /></Button></DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800">
                              <DropdownMenuItem className="text-zinc-300 text-xs" onClick={() => copyVendorLink(inv.token)}>
                                <Copy className="h-3 w-3 mr-2" /> Copy Link
                              </DropdownMenuItem>
                              {detail.status === 'published' && inv.status === 'pending' && (
                                <DropdownMenuItem className="text-zinc-300 text-xs" onClick={() => resendInvMut.mutate({ brId: detail.id, invId: inv.id })}>
                                  <RotateCw className="h-3 w-3 mr-2" /> Resend Email
                                </DropdownMenuItem>
                              )}
                              {!inv.has_submission && detail.status === 'draft' && (
                                <DropdownMenuItem className="text-red-400 text-xs" onClick={() => removeInvMut.mutate({ brId: detail.id, invId: inv.id })}>
          ...
          <TabsContent value="submissions" className="space-y-4">
            {submissions.length === 0 ? (
              <div className="text-center py-12 text-zinc-600 text-sm">No submissions received yet</div>
            ) : (
              <>
                {/* Submission Analytics */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
                    <span className="text-[10px] font-mono text-zinc-500 block">AVERAGE</span>
                    <span className="text-white text-sm font-mono">{detail.currency} {avgPrice.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  </CardContent></Card>
                  <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
                    <span className="text-[10px] font-mono text-zinc-500 block">LOWEST</span>
                    <span className="text-green-400 text-sm font-mono">{detail.currency} {minPrice.toLocaleString()}</span>
                  </CardContent></Card>
                  <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
                    <span className="text-[10px] font-mono text-zinc-500 block">HIGHEST</span>
                    <span className="text-red-400 text-sm font-mono">{detail.currency} {maxPrice.toLocaleString()}</span>
                  </CardContent></Card>
                  <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3">
                    <span className="text-[10px] font-mono text-zinc-500 block">SHORTLISTED</span>
                    <span className="text-amber-400 text-sm font-mono">{shortlisted} / {submissions.length}</span>
                  </CardContent></Card>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-zinc-800">
                      <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">VENDOR</th>
                      <th className="text-right py-2 px-3 text-[10px] font-mono text-zinc-500">BID PRICE</th>
                      <th className="text-right py-2 px-3 text-[10px] font-mono text-zinc-500">TIMELINE</th>
                      <th className="text-center py-2 px-3 text-[10px] font-mono text-zinc-500">SCORE</th>
                      <th className="text-center py-2 px-3 text-[10px] font-mono text-zinc-500">STATUS</th>
                      <th className="text-right py-2 px-3 text-[10px] font-mono text-zinc-500">ACTIONS</th>
                    </tr></thead>
                    <tbody>
                      {submissions
                        ...
                        .map((sub, idx) => {
                          ...
                          return (
                        <tr key={sub.id} className={`border-b border-zinc-800/50 hover:bg-zinc-900/50 ${idx === 0 ? 'bg-green-500/5' : ''}`}>
                          <td className="py-2 px-3">
                            <div className="text-white text-sm">{sub.vendor_name} {idx === 0 && <span className="text-green-400 text-[10px] ml-1">LOWEST</span>}</div>
                            <div className="text-zinc-500 text-xs">{sub.vendor_company}</div>
                          </td>
                          <td className="py-2 px-3 text-right">
                            <span className="text-white font-mono">{sub.currency} {Number(sub.total_bid_price).toLocaleString()}</span>
                            {detail.estimated_budget && (
                              <div className={`text-[10px] font-mono ${Number(sub.total_bid_price) <= Number(detail.estimated_budget) ? 'text-green-400' : 'text-red-400'}`}>
                                {((Number(sub.total_bid_price) / Number(detail.estimated_budget) - 1) * 100).toFixed(1)}% vs budget
                              </div>
                            )}
                          </td>
                          <td className="py-2 px-3 text-right text-zinc-300 font-mono">{sub.timeline_weeks ? `${sub.timeline_weeks}w` : '—'}</td>
                          <td className="py-2 px-3 text-center">
                            {sub.score != null ? <span className="text-amber-400 font-mono">{sub.score}/100</span> : <span className="text-zinc-600">—</span>}
                          </td>
                          <td className="py-2 px-3 text-center">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800">
                                {sub.status !== 'shortlisted' && sub.status !== 'awarded' && (
                                  <DropdownMenuItem className="text-amber-400 text-xs" onClick={() => updateSubStatusMut.mutate({ brId: detail.id, subId: sub.id, status: 'shortlisted' })}>
                                    <Star className="h-3 w-3 mr-2" /> Shortlist
                                  </DropdownMenuItem>
                                )}
                                {sub.status !== 'rejected' && sub.status !== 'awarded' && (
                                  <DropdownMenuItem className="text-red-400 text-xs" onClick={() => updateSubStatusMut.mutate({ brId: detail.id, subId: sub.id, status: 'rejected' })}>
                ...
                <div className="space-y-3 mt-4">
                  <h3 className="text-xs font-mono text-zinc-500">DETAILED SUBMISSIONS</h3>
                  {submissions.map(sub => (
                    <Card key={sub.id} className="bg-zinc-900 border-zinc-800">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            <span className="text-white font-medium text-sm">{sub.vendor_name}</span>
                            <span className="text-zinc-600 text-xs ml-2">({sub.vendor_company})</span>
                          </div>
                          <span className="text-amber-400 font-mono text-sm">{sub.currency} {Number(sub.total_bid_price).toLocaleString()}</span>
                        </div>
                        {sub.line_items?.length > 0 && (
                          <div className="overflow-x-auto mb-3">
                            <table className="w-full text-xs">
                              <thead><tr className="border-b border-zinc-800">
                                <th className="text-left py-1 px-2 text-[10px] font-mono text-zinc-600">ITEM</th>
                                <th className="text-right py-1 px-2 text-[10px] font-mono text-zinc-600">QTY</th>
                                <th className="text-left py-1 px-2 text-[10px] font-mono text-zinc-600">UNIT</th>
                                <th className="text-right py-1 px-2 text-[10px] font-mono text-zinc-600">RATE</th>
                                <th className="text-right py-1 px-2 text-[10px] font-mono text-zinc-600">TOTAL</th>
                              </tr></thead>
                              <tbody>
                                {sub.line_items.map((li: any, i: number) => (
                                  <tr key={i} className="border-b border-zinc-800/30">
                                    <td className="py-1 px-2 text-zinc-300">{li.description}</td>
                                    <td className="py-1 px-2 text-zinc-400 text-right">{li.qty}</td>
                                    <td className="py-1 px-2 text-zinc-500">{li.unit}</td>
                                    <td className="py-1 px-2 text-zinc-300 text-right">{Number(li.unit_rate).toLocaleString()}</td>
                                    <td className="py-1 px-2 text-white text-right">{Number(li.total).toLocaleString()}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                          {sub.materials_specification && (
                            <div><span className="text-zinc-500 font-mono block text-[10px]">MATERIALS</span><span className="text-zinc-300">{sub.materials_specification}</span></div>
                          )}
                          {sub.notes && (
                            <div><span className="text-zinc-500 font-mono block text-[10px]">NOTES</span><span className="text-zinc-300">{sub.notes}</span></div>
                          )}
                          {sub.value_engineering_suggestions && (
                            <div><span className="text-zinc-500 font-mono block text-[10px]">VALUE ENGINEERING</span><span className="text-zinc-300">{sub.value_engineering_suggestions}</span></div>
                          )}
                        </div>
                        {/* Submission Attachments */}
                        {sub.attachments?.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-zinc-800">
                            <span className="text-zinc-500 font-mono block text-[10px] mb-2">ATTACHMENTS ({sub.attachments.length})</span>
                            <div className="flex flex-wrap gap-2">
                              {sub.attachments.map((att: any, i: number) => (
                                <a
                                  ...
                                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded bg-zinc-800 border border-zinc-700 hover:border-amber-500/50 transition-colors group ${!att.download_url ? 'pointer-events-none opacity-50' : ''}`}
                                >
                                  <Paperclip className="h-3 w-3 text-zinc-500 group-hover:text-amber-400" />
                                  <span className="text-xs text-zinc-300 group-hover:text-white max-w-[160px] truncate">{att.file_name}</span>
                                  <Download className="h-3 w-3 text-zinc-600 group-hover:text-amber-400" />
                                </a>
                              ))}
                            </div>
                          </div>
          ...
          <TabsContent value="qa" className="space-y-4">
            {qa.length === 0 ? (
              <div className="text-center py-12 text-zinc-600 text-sm">No questions yet</div>
            ) : (
                      ...
                      {thread.answer ? (
                        ...
                        <div className="ml-6 mt-3">
                          {answeringId === thread.id ? (
                            ...
                            <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-400 text-xs" onClick={() => setAnsweringId(thread.id)}>
                              Answer
                            </Button>
                          )}
                        </div>
          ...
          <TabsContent value="attachments" className="space-y-4">
            ...
            {(detail.attachments?.length || 0) === 0 ? (
              <div className="text-center py-12 text-zinc-600 text-sm">No attachments</div>
            ) : (
                ...
                {detail.attachments.map((att: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-zinc-900 border border-zinc-800 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-white text-sm truncate block">{att.file_name}</span>
                      <span className="text-zinc-600 text-xs">{att.file_size ? `${(att.file_size / 1024).toFixed(0)} KB` : ''}</span>
                    </div>
                    {att.download_url && (
                      <Button size="sm" variant="ghost" className="text-zinc-400 text-xs" onClick={() => window.open(att.download_url, '_blank')}>Download</Button>
                    )}
                  </div>
                ))}
              </div>
          ...
          <TabsContent value="activity" className="space-y-4">
            {activity.length === 0 ? (
              <div className="text-center py-12 text-zinc-600 text-sm">No activity recorded</div>
            ) : (
              <div className="space-y-2">
                {activity.map(log => (
                  <div key={log.id} className="flex items-start gap-3 p-2">
                    ...
                    <div className="flex-1">
                      <p className="text-zinc-300 text-sm">{log.action.replace(/_/g, ' ')}</p>
                      <p className="text-zinc-600 text-[10px] font-mono">{log.actor_name || 'System'} · {new Date(log.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Dialog open={showInvDialog} onOpenChange={setShowInvDialog}>
          <DialogContent className="bg-zinc-900 border-zinc-800 max-w-md">
            <DialogHeader><DialogTitle className="text-white">Invite Vendor</DialogTitle></DialogHeader>
            <div className="space-y-3">
        ...
        <Dialog open={showAwardDialog} onOpenChange={setShowAwardDialog}>
          <DialogContent className="bg-zinc-900 border-zinc-800 max-w-md">
            <DialogHeader><DialogTitle className="text-white flex items-center gap-2"><Trophy className="h-4 w-4 text-green-400" /> Award Bid</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div>
                <Label className="text-[10px] font-mono text-zinc-500">SELECT WINNING SUBMISSION</Label>
                <Select value={awardSubId} onValueChange={setAwardSubId}>
                  <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white"><SelectValue placeholder="Choose vendor..." /></SelectTrigger>
              ...
              <div><Label className="text-[10px] font-mono text-zinc-500">AWARD NOTES</Label><Textarea className="bg-zinc-800 border-zinc-700 text-white" rows={3} value={awardNotes} onChange={e => setAwardNotes(e.target.value)} placeholder="Reason for selection..." /></div>
              <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
                <p className="text-amber-400 text-xs"><AlertTriangle className="h-3 w-3 inline mr-1" />This action is final. The winning vendor will be notified and all other vendors will be informed they were not selected.</p>
              </div>
              <Button className="w-full bg-green-600 hover:bg-green-700 text-white" disabled={awardMut.isPending || !awardSubId}
  ...
  return (
    <div className="space-y-4">
      ...
      {/* KPI Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center"><Gavel className="h-4 w-4 text-zinc-400" /></div>
          <div><span className="text-[10px] font-mono text-zinc-500 block">TOTAL</span><span className="text-white text-lg font-mono">{kpi.total}</span></div>
        </CardContent></Card>
        <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center"><FileText className="h-4 w-4 text-zinc-500" /></div>
          <div><span className="text-[10px] font-mono text-zinc-500 block">DRAFTS</span><span className="text-white text-lg font-mono">{kpi.draft}</span></div>
        </CardContent></Card>
        <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center"><Send className="h-4 w-4 text-blue-400" /></div>
          <div><span className="text-[10px] font-mono text-zinc-500 block">ACTIVE</span><span className="text-blue-400 text-lg font-mono">{kpi.active}</span></div>
        </CardContent></Card>
        <Card className="bg-zinc-900 border-zinc-800"><CardContent className="p-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center"><Trophy className="h-4 w-4 text-green-400" /></div>
          <div><span className="text-[10px] font-mono text-zinc-500 block">AWARDED</span><span className="text-green-400 text-lg font-mono">{kpi.awarded}</span></div>
        </CardContent></Card>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search bids..." className="pl-8 bg-zinc-900 border-zinc-800 text-white text-sm h-9" />
        </div>
        <Select value={projectFilter || 'all'} onValueChange={v => setProjectFilter(v === 'all' ? '' : v)}>
          <SelectTrigger className="bg-zinc-900 border-zinc-800 text-white text-xs w-[160px] h-9"><SelectValue placeholder="All Projects" /></SelectTrigger>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="bg-zinc-900 border-zinc-800 text-white text-xs w-[130px] h-9"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-zinc-900 border-zinc-800">
        ...
        <Select value={tradeFilter} onValueChange={setTradeFilter}>
          <SelectTrigger className="bg-zinc-900 border-zinc-800 text-white text-xs w-[130px] h-9"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-zinc-900 border-zinc-800">
            <SelectItem value="all" className="text-zinc-300">All Trades</SelectItem>
            {TRADE_CATEGORIES.map(t => <SelectItem key={t} value={t} className="text-white capitalize">{t.replace(/_/g, ' ')}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      {loadingList ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-zinc-800">
              <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">TITLE</th>
              <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">PROJECT</th>
              <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">TRADE</th>
              <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">STATUS</th>
              <th className="text-right py-2 px-3 text-[10px] font-mono text-zinc-500">BUDGET</th>
              <th className="text-center py-2 px-3 text-[10px] font-mono text-zinc-500">VENDORS</th>
              <th className="text-center py-2 px-3 text-[10px] font-mono text-zinc-500">BIDS</th>
              <th className="text-left py-2 px-3 text-[10px] font-mono text-zinc-500">DEADLINE</th>
            </tr></thead>
            <tbody>
              {list.map(br => {
                const s = STATUS_MAP[br.status] || STATUS_MAP.draft
                return (
                  <tr key={br.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/60 cursor-pointer transition" onClick={() => setSelectedId(br.id)}>
                    <td className="py-2.5 px-3">
                      <span className="text-white font-medium">{br.title}</span>
                    </td>
                    <td className="py-2.5 px-3 text-zinc-400 text-xs">{br.project_name}</td>
                    <td className="py-2.5 px-3 text-zinc-400 text-xs capitalize">{(br.trade_category || '').replace(/_/g, ' ')}</td>
                    <td className="py-2.5 px-3">
      ...
      <Dialog open={showCreate} onOpenChange={v => { setShowCreate(v); if (!v) setEditingId(null) }}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="text-white">{editingId ? 'Edit Bid Request' : 'New Bid Request'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <div>
              ...
              <Input className="bg-zinc-800 border-zinc-700 text-white" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. Electrical Works — Phase 1" />
            </div>
            <div>
              <Label className="text-[10px] font-mono text-zinc-500">DESCRIPTION / SCOPE</Label>
              <Textarea className="bg-zinc-800 border-zinc-700 text-white" rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Detailed scope of work..." />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-[10px] font-mono text-zinc-500">TRADE CATEGORY</Label>
              ...
              <div>
                <Label className="text-[10px] font-mono text-zinc-500">ESTIMATED BUDGET</Label>
                <div className="flex">
                  <Select value={form.currency} onValueChange={v => setForm({ ...form, currency: v })}>
                    <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white w-20 rounded-r-none border-r-0"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-zinc-800 border-zinc-700">
                  ...
                  <Input type="number" className="bg-zinc-800 border-zinc-700 text-white rounded-l-none flex-1" value={form.estimated_budget} onChange={e => setForm({ ...form, estimated_budget: e.target.value })} placeholder="0.00" />
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-[10px] font-mono text-zinc-500">SUBMISSION DEADLINE *</Label>
                <Input type="datetime-local" className="bg-zinc-800 border-zinc-700 text-white" value={form.submission_deadline} onChange={e => setForm({ ...form, submission_deadline: e.target.value })} />
              </div>
              <div>
                <Label className="text-[10px] font-mono text-zinc-500">Q&A DEADLINE</Label>
                <Input type="datetime-local" className="bg-zinc-800 border-zinc-700 text-white" value={form.qa_deadline} onChange={e => setForm({ ...form, qa_deadline: e.target.value })} />
              </div>
            </div>
            <div>
              <Label className="text-[10px] font-mono text-zinc-500">SITE VISIT DATE</Label>
              <Input type="datetime-local" className="bg-zinc-800 border-zinc-700 text-white" value={form.site_visit_date} onChange={e => setForm({ ...form, site_visit_date: e.target.value })} />
            </div>
            <div>
              <Label className="text-[10px] font-mono text-zinc-500">VENDOR ELIGIBILITY NOTES</Label>
              <Textarea className="bg-zinc-800 border-zinc-700 text-white" rows={2} value={form.vendor_eligibility_notes} onChange={e => setForm({ ...form, vendor_eligibility_notes: e.target.value })} placeholder="Minimum qualifications, certifications required..." />
            </div>
            <div className="flex items-center gap-2">
