      ...
      <ScrollArea className="flex-1">
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <MetricCard
              title="Running Jobs"
              value={formatNumber(jobStats?.data?.by_status?.running || 0)}
              subtitle={`${(jobStats?.data?.by_status?.completed || 0)} completed`}
              icon={GitBranch}
              color="green"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  <div className="text-center p-4 rounded-lg bg-green-500/10">
                    <p className="text-2xl font-bold text-green-400">
                      {jobStats?.data?.by_status?.completed || 0}
                    </p>
                    <p className="text-sm text-muted-foreground">Completed</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-blue-500/10">
                    <p className="text-2xl font-bold text-blue-400">
                      {jobStats?.data?.by_status?.running || 0}
                    </p>
                    <p className="text-sm text-muted-foreground">Running</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-yellow-500/10">
                    <p className="text-2xl font-bold text-yellow-400">
                      {jobStats?.data?.by_status?.pending || 0}
                    </p>
                    <p className="text-sm text-muted-foreground">Pending</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-red-500/10">
                    <p className="text-2xl font-bold text-red-400">
                      {jobStats?.data?.by_status?.failed || 0}
                    </p>
                    <p className="text-sm text-muted-foreground">Failed</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-purple-500/10">
                    <p className="text-2xl font-bold text-purple-400">
                      {jobStats?.data?.by_status?.cancelled || 0}
                    </p>
                    <p className="text-sm text-muted-foreground">Cancelled</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-muted/50">
                    <p className="text-2xl font-bold">
                      {((jobStats?.data?.by_status?.completed || 0) / Math.max((jobStats?.data?.by_status?.completed || 0) + (jobStats?.data?.by_status?.failed || 0), 1) * 100).toFixed(0)}%
                    </p>
