...
function PSIBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-[10px] text-zinc-300 w-12 text-right tabular-nums">{value.toFixed(4)}</span>
    </div>
  )
}
        ...
        <Panel title="DRIFTED FEATURES">
          <div className="text-center py-1">
            <div className="font-mono text-3xl text-white">
              ...
              <span className="text-zinc-600 text-lg"> / {featureDrift.length}</span>
            </div>
            <div className="font-mono text-[10px] text-zinc-500">FEATURES WITH DRIFT</div>
          </div>
        </Panel>
        ...
        <Panel title="PERFORMANCE TREND">
          <div className="text-center py-1">
            {trend ? (
              ...
              <div className="font-mono text-[10px] text-zinc-600">No trend data</div>
            )}
          </div>
        </Panel>
      </div>
      ...
      <div className="grid grid-cols-12 gap-3 mb-4">
        ...
        <Panel title="PER-FEATURE DATA DRIFT" className="col-span-8">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="font-mono text-[10px] text-zinc-500 text-left py-1.5 px-2">FEATURE</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">PSI</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">KS STAT</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">CHANGE %</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-center py-1.5 px-2">DRIFT</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-center py-1.5 px-2">SEVERITY</th>
                </tr>
              </thead>
              <tbody>
                {featureDrift
                  ...
                  .map((f) => (
                    <tr key={f.feature_name} className="border-b border-zinc-800/50 hover:bg-zinc-800/20">
                      ...
                      <td className="font-mono text-[10px] text-zinc-300 text-right py-1.5 px-2 tabular-nums">
                        {f.ks_statistic.toFixed(4)}
                      </td>
                      <td className="font-mono text-[10px] text-right py-1.5 px-2 tabular-nums">
                        {f.change_pct !== undefined ? (
                          <span className={f.change_pct >= 0 ? 'text-green-400' : 'text-red-400'}>
          ...
          {featureDrift.length === 0 && (
            <div className="font-mono text-[10px] text-zinc-600 text-center py-6">No feature drift data</div>
          )}
        </Panel>
          ...
          {drift ? (
            <div className="space-y-3">
              <div>
                ...
                <div className="font-mono text-[10px] text-zinc-300 leading-relaxed p-2 border border-zinc-800 bg-zinc-900">
              ...
              {drift.metrics && Object.keys(drift.metrics).length > 0 && (
                <div>
                  <div className="font-mono text-[9px] text-zinc-500 mb-1">AGGREGATE METRICS</div>
              ...
              <div className="border-t border-zinc-800 pt-2">
                <div className="font-mono text-[9px] text-zinc-500 mb-1">PSI REFERENCE</div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2 font-mono text-[10px]">
                    <div className="w-3 h-2 bg-green-500/70 rounded" />
                    <span className="text-zinc-400">&lt; 0.1</span>
                    <span className="text-zinc-600">No drift</span>
                  </div>
                  <div className="flex items-center gap-2 font-mono text-[10px]">
                    <div className="w-3 h-2 bg-amber-500/70 rounded" />
                    <span className="text-zinc-400">0.1 – 0.2</span>
                    <span className="text-zinc-600">Moderate drift</span>
                  </div>
                  <div className="flex items-center gap-2 font-mono text-[10px]">
                    <div className="w-3 h-2 bg-red-500/70 rounded" />
                    <span className="text-zinc-400">&gt; 0.2</span>
                    <span className="text-zinc-600">Significant drift</span>
                  </div>
                </div>
              </div>
            </div>
          ) : (
            <div className="font-mono text-[10px] text-zinc-600 text-center py-6">No drift data</div>
          )}
        </Panel>
      </div>
        ...
        >
          <div className="space-y-2">
            {trend && trendPoints.length > 0 ? (
              ...
              <div className="font-mono text-[10px] text-zinc-600 text-center py-8">No trend data</div>
            )}
          </div>
        </Panel>
              ...
              segments.slice(0, 8).map((seg) => (
                <div key={`${seg.segment_type}-${seg.segment_name}`} className="space-y-0.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-[9px] text-zinc-600">{seg.segment_type}</span>
                  </div>
                  <div className="flex items-center gap-3 font-mono text-[9px]">
                    <span className="text-zinc-500">MAPE: <span className="text-white">{seg.mape.toFixed(1)}%</span></span>
                    <span className="text-zinc-500">R²: <span className="text-white">{seg.r2.toFixed(3)}</span></span>
                    <span className="text-zinc-500">n={seg.sample_size}</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="font-mono text-[10px] text-zinc-600 text-center py-6">No segment data</div>
            )}
          </div>
        </Panel>
      </div>
        ...
        <Panel title="SEGMENT PERFORMANCE BREAKDOWN">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="font-mono text-[10px] text-zinc-500 text-left py-1.5 px-2">SEGMENT</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-left py-1.5 px-2">TYPE</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">MAE</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">MAPE</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">R²</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">±10%</th>
                  <th className="font-mono text-[10px] text-zinc-500 text-right py-1.5 px-2">SAMPLE</th>
                </tr>
              </thead>
              <tbody>
                {segments.map((seg) => (
                  ...
                  >
                    ...
                    <td className="py-1.5 px-2">
                      <span className="px-1.5 py-0.5 font-mono text-[9px] text-zinc-400 border border-zinc-700 rounded">
                        {seg.segment_type.toUpperCase()}
                      </span>
                    </td>
                    <td className="font-mono text-[10px] text-white text-right py-1.5 px-2 tabular-nums">
                    ...
                    <td className={cn(
                      ...
                      seg.r2 > 0.85 ? 'text-green-400' : seg.r2 > 0.7 ? 'text-amber-400' : 'text-red-400'
                    )}>
                      {seg.r2.toFixed(4)}
                    </td>
                    <td className="font-mono text-[10px] text-white text-right py-1.5 px-2 tabular-nums">
                    ...
                    <td className="font-mono text-[10px] text-zinc-400 text-right py-1.5 px-2 tabular-nums">
