...
function TrendBadge({ direction }: { direction: string }) {
  ...
  return (
    <span className={cn(
      ...
      isUp ? 'text-green-400 bg-green-500/10 border-green-500/20' :
...
function CategoryBadge({ category }: { category: string }) {
  const colors: Record<string, string> = {
    affordable: 'text-green-400 bg-green-500/10 border-green-500/20',
    moderate: 'text-amber-400 bg-amber-500/10 border-amber-500/20',
    unaffordable: 'text-orange-400 bg-orange-500/10 border-orange-500/20',
    severely_unaffordable: 'text-red-400 bg-red-500/10 border-red-500/20',
  }
  return (
    <span className={cn(
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex justify-between font-mono text-[9px]">
        <span className="text-zinc-500">START: <span className="text-white">{valuePrefix}{forecast[0]?.predicted.toFixed(1)}{valueSuffix}</span></span>
        <span className="text-zinc-500">END: <span className="text-amber-400">{valuePrefix}{forecast[forecast.length - 1]?.predicted.toFixed(1)}{valueSuffix}</span></span>
      </div>
    </div>
  )
}
      ...
      <div className="min-h-screen bg-black text-white p-4">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-12 gap-3">
            <div className="col-span-6 h-80 bg-zinc-800/50 rounded border border-zinc-800" />
            <div className="col-span-6 h-80 bg-zinc-800/50 rounded border border-zinc-800" />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {showRegionDropdown && (
              <div className="absolute right-0 top-full mt-1 z-50 bg-zinc-900 border border-zinc-700 shadow-lg max-h-48 overflow-y-auto">
                {regions.map(r => (
                  <button
                    key={r}
                    ...
                    className={cn(
                      ...
                      r === selectedRegion ? 'text-amber-500 bg-zinc-800' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/50'
                    )}
                  >
        ...
        <Panel title="CCI FORECAST">
          <div className="text-center py-1">
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="font-mono text-[10px] text-zinc-500">{horizon}M AHEAD</span>
            </div>
        ...
        <Panel title="CCI CONFIDENCE">
          <div className="text-center py-1">
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="font-mono text-[10px] text-zinc-500">INTERVAL</span>
            </div>
        ...
        <Panel title="MODEL QUALITY">
          <div className="text-center py-1">
            <div className="flex items-center justify-center gap-1 mb-1">
              ...
              <span className="font-mono text-[10px] text-zinc-500">R² SCORE</span>
            </div>
            <div className="space-y-1 mt-1">
              <div className="flex justify-between font-mono text-[10px]">
                <span className="text-zinc-500">CCI</span>
                <span className={cn(
                  (cciForecast?.model_info.r_squared ?? 0) >= 0.7 ? 'text-green-400' :
                  (cciForecast?.model_info.r_squared ?? 0) >= 0.4 ? 'text-amber-400' :
                  'text-red-400'
                )}>
              ...
              <div className="flex justify-between font-mono text-[10px]">
                <span className="text-zinc-500">GHAI</span>
                <span className={cn(
                  (ghaiForecast?.model_info.r_squared ?? 0) >= 0.7 ? 'text-green-400' :
                  (ghaiForecast?.model_info.r_squared ?? 0) >= 0.4 ? 'text-amber-400' :
                  'text-red-400'
                )}>
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-zinc-800 pt-2 space-y-1">
                <div className="flex justify-between font-mono text-[10px]">
                  ...
                  <span className="text-zinc-300">{cciForecast.model_info.method.replace(/_/g, ' ').toUpperCase()}</span>
                </div>
                <div className="flex justify-between font-mono text-[10px]">
                  <span className="text-zinc-500">DATA POINTS</span>
                  <span className="text-white">{cciForecast.model_info.data_points}</span>
                </div>
                <div className="flex justify-between font-mono text-[10px]">
                  <span className="text-zinc-500">MONTHLY CHANGE</span>
                  <span className={cciForecast.model_info.monthly_change_rate >= 0 ? 'text-red-400' : 'text-green-400'}>
              ...
              <div className="border-t border-zinc-800 pt-2">
                <div className="font-mono text-[9px] text-zinc-500 mb-2">COMPONENT FORECASTS</div>
                  ...
                  {(['materials', 'labor', 'overhead'] as const).map(comp => {
                    ...
                    return (
                      <div key={comp} className="space-y-1">
                        <div className="font-mono text-[9px] text-zinc-500">{comp.toUpperCase()}</div>
                        {compData.length > 0 && (
                          <>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-zinc-800">
                        <th className="font-mono text-[9px] text-zinc-500 text-left py-1 px-1">DATE</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">PREDICTED</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">LOWER</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">UPPER</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">CONF</th>
                      </tr>
                    </thead>
                    <tbody>
                      {cciForecast.forecast.map((f, i) => (
                        <tr key={i} className="border-b border-zinc-800/30">
                          <td className="font-mono text-[9px] text-zinc-400 py-1 px-1">{f.period_date}</td>
                          <td className="font-mono text-[9px] text-amber-400 text-right py-1 px-1">{f.predicted_index.toFixed(1)}</td>
                          <td className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">{f.lower_bound.toFixed(1)}</td>
                          <td className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">{f.upper_bound.toFixed(1)}</td>
                          <td className="font-mono text-[9px] text-zinc-400 text-right py-1 px-1">{(f.confidence * 100).toFixed(0)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {ghaiForecast ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-2 border border-zinc-800 rounded">
                <div>
                  <div className="font-mono text-[9px] text-zinc-500">CURRENT</div>
                  <div className="font-mono text-lg text-white">
                ...
                <div className="text-right">
                  <div className="font-mono text-[9px] text-zinc-500">{horizon}M FORECAST</div>
              ...
              {/* Sub-index forecasts */}
              <div className="border-t border-zinc-800 pt-2">
                <div className="font-mono text-[9px] text-zinc-500 mb-2">SUB-INDEX FORECASTS</div>
                <div className="grid grid-cols-3 gap-3">
                  {(['mhai', 'chai', 'rhai'] as const).map(idx => {
                    ...
                    const colors: Record<string, string> = {
                      mhai: 'text-blue-400',
                      chai: 'text-green-400',
                      rhai: 'text-purple-400',
                    }
                    return (
                      <div key={idx} className="space-y-1">
                        <div className="font-mono text-[9px] text-zinc-500">{labels[idx]}</div>
                        {subData.length > 0 && (
                          <>
              ...
              <div className="border-t border-zinc-800 pt-2 space-y-1">
                <div className="flex justify-between font-mono text-[10px]">
                  <span className="text-zinc-500">METHOD</span>
                  <span className="text-zinc-300">{ghaiForecast.model_info.method.replace(/_/g, ' ').toUpperCase()}</span>
                </div>
                <div className="flex justify-between font-mono text-[10px]">
                  <span className="text-zinc-500">DATA POINTS</span>
                  <span className="text-white">{ghaiForecast.model_info.data_points}</span>
                </div>
                <div className="flex justify-between font-mono text-[10px]">
                  <span className="text-zinc-500">MONTHLY TREND</span>
                ...
                <div className="overflow-x-auto max-h-36 overflow-y-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-zinc-800">
                        <th className="font-mono text-[9px] text-zinc-500 text-left py-1 px-1">DATE</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">GHAI</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">LOWER</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">UPPER</th>
                        <th className="font-mono text-[9px] text-zinc-500 text-left py-1 px-1">CATEGORY</th>
                      </tr>
                    </thead>
                    <tbody>
                      {ghaiForecast.forecast.map((f, i) => (
                        <tr key={i} className="border-b border-zinc-800/30">
                          <td className="font-mono text-[9px] text-zinc-400 py-1 px-1">{f.period_date}</td>
                          <td className="font-mono text-[9px] text-amber-400 text-right py-1 px-1">{f.predicted_ghai.toFixed(1)}</td>
                          <td className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">{f.lower_bound.toFixed(1)}</td>
                          <td className="font-mono text-[9px] text-zinc-500 text-right py-1 px-1">{f.upper_bound.toFixed(1)}</td>
