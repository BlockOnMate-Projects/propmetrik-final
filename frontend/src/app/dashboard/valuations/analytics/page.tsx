    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            if (valuationsRes?.ok) {
                ...
                const typeColors = ['text-green-600 dark:text-green-400 bg-green-500/10', 'text-blue-600 dark:text-blue-400 bg-blue-500/10', 'text-purple-600 dark:text-purple-400 bg-purple-500/10', 'text-amber-600 dark:text-amber-400 bg-amber-500/10', 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/10']
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-4 pb-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                <div>
                    <h1 className="font-mono text-lg sm:text-xl text-foreground flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={fetchAnalytics} className="flex items-center gap-2 px-3 py-2 bg-muted text-muted-foreground font-mono text-xs hover:text-foreground transition-colors">
            ...
            <div className="flex gap-1 mb-6 border-b border-border">
                <button
                    ...
                    className={`px-4 py-2 font-mono text-xs border-b-2 transition-colors ${activeTab === 'overview' ? 'text-amber-500 border-amber-500' : 'text-muted-foreground border-transparent hover:text-muted-foreground'}`}
                >
                    <BarChart3 className="w-3 h-3 inline mr-2" />OVERVIEW
                </button>
                <button
                    onClick={() => setActiveTab('firm')}
                    className={`px-4 py-2 font-mono text-xs border-b-2 transition-colors ${activeTab === 'firm' ? 'text-amber-500 border-amber-500' : 'text-muted-foreground border-transparent hover:text-muted-foreground'}`}
                >
            ...
            {loading ? (
                <div className="flex items-center justify-center py-20">
                    ...
                    <span className="ml-3 font-mono text-xs text-muted-foreground">Loading analytics...</span>
                </div>
            ) : activeTab === 'overview' ? (
            <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mb-6">
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">TOTAL VALUATIONS</div>
                    <div className="font-mono text-2xl text-foreground">{totalValuations}</div>
                ...
                <div className="bg-card border border-border p-4">
                    ...
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">{corporateCount} corporate</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">INVOICES</div>
                    <div className="font-mono text-2xl text-blue-600 dark:text-blue-400">{summary?.invoiceCount || 0}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">{summary?.paidCount || 0} paid</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">TOTAL INVOICED</div>
                    <div className="font-mono text-2xl text-green-600 dark:text-green-400">{formatCurrency(summary?.totalInvoiced || 0)}</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">OUTSTANDING</div>
                    <div className="font-mono text-2xl text-red-600 dark:text-red-400">{formatCurrency(summary?.totalOutstanding || 0)}</div>
                    <div className="flex items-center gap-1 mt-1">
                        {(summary?.overdueCount || 0) > 0 && <>
                            <AlertTriangle className="w-3 h-3 text-red-600 dark:text-red-400" />
                            <span className="font-mono text-[10px] text-red-600 dark:text-red-400">{summary?.overdueCount} overdue</span>
                        </>}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ...
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">OVERDUE AMOUNT</div>
                ...
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">AVG PER CLIENT</div>
                    <div className="font-mono text-xl text-amber-600 dark:text-amber-400">{formatCurrency(avgBilled)}</div>
                </div>
            </div>
                ...
                <div className="bg-card border border-border p-4 col-span-2">
                    ...
                    {clients.length === 0 ? (
                        <div className="text-center py-6">
                            ...
                            <p className="font-mono text-xs text-muted-foreground">No client data available</p>
                        </div>
                    ) : (
                            ...
                            <tbody className="font-mono text-xs">
                                {clients.sort((a, b) => (b.totalBilled || 0) - (a.totalBilled || 0)).slice(0, 8).map(c => (
                                    <tr key={c.id} className="border-b border-border/50">
                                        ...
                                        <td className="py-2 text-right text-muted-foreground">{c.valuationsCount || 0}</td>
                                        <td className="py-2 text-right text-amber-600 dark:text-amber-400 font-bold">{formatCurrency(c.totalBilled || 0)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                            ...
                            {propertyTypes.map((pt, idx) => (
                                <div key={idx}>
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="font-mono text-xs text-muted-foreground capitalize">{pt.type}</span>
                                        <span className="font-mono text-[10px] text-muted-foreground">{pt.count}</span>
                                    </div>
                        ...
                        <tbody className="font-mono text-xs">
                            {regions.map((r, idx) => (
                                <tr key={idx} className="border-b border-border/50">
                                    <td className="py-2 text-foreground capitalize">{r.region}</td>
                                    ...
                                    <td className="py-2 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <span className="text-muted-foreground text-[10px] w-8 text-right">{r.percentage}%</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="bg-card border border-border p-4">
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="font-mono text-xs">
                            {methods.map((m, idx) => (
                                <tr key={idx} className="border-b border-border/50">
                                    <td className="py-2 text-foreground capitalize">{m.method}</td>
                                    ...
                                    <td className="py-2 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <span className="text-muted-foreground text-[10px] w-8 text-right">{m.percentage}%</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            {firmLoading ? (
                <div className="flex items-center justify-center py-20">
                    <Loader2 className="w-6 h-6 animate-spin text-amber-500" />
                    <span className="ml-3 font-mono text-xs text-muted-foreground">Loading firm analytics...</span>
                </div>
            ) : firmData ? (
            <>
            {/* Period Selector */}
            <div className="flex items-center gap-2 mb-6">
                <span className="font-mono text-[10px] text-muted-foreground">PERIOD:</span>
                {['7d', '30d', '90d', '1y'].map(p => (
                    <button
                        key={p}
                        onClick={() => setFirmPeriod(p)}
                        className={`px-3 py-1.5 font-mono text-xs transition-colors ${firmPeriod === p ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30' : 'bg-muted text-muted-foreground border border-border hover:text-muted-foreground'}`}
                    >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">REVENUE</div>
                    <div className="font-mono text-2xl text-green-600 dark:text-green-400">{formatCurrency(firmData.revenue.total)}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">{firmData.revenue.paid_count} paid</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">OUTSTANDING</div>
                    <div className="font-mono text-2xl text-amber-600 dark:text-amber-400">{formatCurrency(firmData.revenue.outstanding)}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">{firmData.revenue.pending_count} pending</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">OVERDUE</div>
                    <div className="font-mono text-2xl text-red-600 dark:text-red-400">{formatCurrency(firmData.revenue.overdue)}</div>
                    {firmData.revenue.overdue_count > 0 && (
                        <div className="flex items-center gap-1 mt-1">
                            <AlertTriangle className="w-3 h-3 text-red-600 dark:text-red-400" />
                            <span className="font-mono text-[10px] text-red-600 dark:text-red-400">{firmData.revenue.overdue_count}</span>
                        </div>
                    )}
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">VALUATIONS</div>
                    <div className="font-mono text-2xl text-foreground">{firmData.throughput.total}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">{firmData.throughput.completed} completed</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">AVG TURNAROUND</div>
                    <div className="font-mono text-2xl text-blue-600 dark:text-blue-400">{firmData.throughput.avg_turnaround_days}d</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">completion time</div>
                </div>
                <div className="bg-card border border-border p-4">
                    <div className="font-mono text-[10px] text-muted-foreground mb-1">ACTIVE CLIENTS</div>
                    <div className="font-mono text-2xl text-amber-500">{firmData.clients.active}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-1">
                        {firmData.clients.new > 0 && <span className="text-green-600 dark:text-green-400">+{firmData.clients.new} new</span>}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="bg-card border border-border p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-mono text-lg text-foreground">{firmData.throughput.completed}</div>
                        <div className="font-mono text-[10px] text-muted-foreground">COMPLETED</div>
                    </div>
                </div>
                <div className="bg-card border border-border p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-mono text-lg text-foreground">{firmData.throughput.in_progress}</div>
                        <div className="font-mono text-[10px] text-muted-foreground">IN PROGRESS</div>
                    </div>
                </div>
                <div className="bg-card border border-border p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-mono text-lg text-foreground">{firmData.throughput.draft}</div>
                        <div className="font-mono text-[10px] text-muted-foreground">DRAFT</div>
                    </div>
                </div>
                <div className="bg-card border border-border p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-mono text-lg text-foreground">{firmData.clients.total}</div>
                        <div className="font-mono text-[10px] text-muted-foreground">TOTAL CLIENTS</div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-card border border-border p-4 col-span-2">
                    ...
                    {firmData.valuers.length === 0 ? (
                        <div className="text-center py-6">
                            <Users className="w-6 h-6 text-zinc-700 mx-auto mb-2" />
                            <p className="font-mono text-xs text-muted-foreground">No valuer data for this period</p>
                        </div>
                    ) : (
                        <table className="w-full">
                            <thead>
                            ...
                            <tbody className="font-mono text-xs">
                                {firmData.valuers.map(v => (
                                    <tr key={v.id} className="border-b border-border/50">
                                        <td className="py-2 text-foreground">{v.name || 'Unknown'}</td>
                                        <td className="py-2 text-muted-foreground capitalize">{v.role.replace(/_/g, ' ')}</td>
                                        <td className="py-2 text-right text-muted-foreground">{v.valuations}</td>
                                        <td className="py-2 text-right text-green-600 dark:text-green-400 font-bold">{v.completed}</td>
                                        <td className="py-2 text-right text-blue-600 dark:text-blue-400">{v.avg_turnaround_days > 0 ? `${v.avg_turnaround_days}d` : '—'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="bg-card border border-border p-4">
                    ...
                    {firmData.monthly_trend.length === 0 ? (
                        <div className="text-center py-6">
                            ...
                            <p className="font-mono text-xs text-muted-foreground">No trend data yet</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                                ...
                                return (
                                    <div key={idx}>
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <span className="font-mono text-[10px] text-muted-foreground">{m.valuations} val · {formatCurrency(m.revenue)}</span>
                                        </div>
                                        <div className="w-full h-2 bg-muted rounded-full flex gap-0.5">
                                            <div
            ...
            {firmData.property_types.length > 0 && (
                        ...
                        {firmData.property_types.map((pt, idx) => {
                            ...
                            return (
                                <div key={idx} className="text-center">
                                    <div className="font-mono text-lg text-foreground">{pt.count}</div>
                                    ...
                                    <div className="font-mono text-[10px] text-muted-foreground capitalize">{(pt.type || 'Unknown').replace(/_/g, ' ')}</div>
                                </div>
                            )
                        })}
                    </div>
                ...
                <div className="text-center py-20">
                    ...
                    <p className="font-mono text-sm text-muted-foreground">No firm analytics data available</p>
                    <p className="font-mono text-[10px] text-muted-foreground mt-1">Ensure the enterprise API is configured</p>
                </div>
            )}
            </>
            )}
