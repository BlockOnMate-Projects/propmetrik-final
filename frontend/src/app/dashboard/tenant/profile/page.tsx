    ...
    <div className="space-y-6 animate-fade-in max-w-3xl">
      ...
      <div className="bg-card rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-cyan-600 to-cyan-500 px-6 py-8">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-card/20 backdrop-blur-sm flex items-center justify-center text-foreground text-2xl font-bold">
              {(profile?.fullName?.[0] || '').toUpperCase()}
            </div>
            <div className="text-foreground">
              <h2 className="text-xl font-bold">{profile?.fullName}</h2>
              <p className="text-cyan-100 text-sm">{profile?.email}</p>
              <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 bg-card/20 rounded-full text-xs font-medium text-foreground">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {!editing ? (
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleCancel} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-gray-700 font-medium">
              ...
              <div className="flex-1">
                ...
                {editing ? (
                  <input type="tel" value={form.phoneSecondary} onChange={e => setForm({ ...form, phoneSecondary: e.target.value })}
                    className="w-full text-sm border border-border rounded-lg px-2 py-1 mt-0.5 focus:ring-2 focus:ring-cyan-500" />
                ) : (
      ...
      <div className="flex items-center gap-3">
        <Link href="/dashboard/tenant/settings"
          className="flex items-center gap-2 px-4 py-2.5 bg-muted hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700 transition-colors">
