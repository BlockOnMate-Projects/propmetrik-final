...
function SettingsContent() {
  ...
  const Toggle = ({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) => (
    <button onClick={onToggle} className={`relative w-11 h-6 rounded-full transition-colors ${enabled ? 'bg-cyan-500' : 'bg-gray-300'}`}>
      <div className={`absolute top-0.5 w-5 h-5 bg-card rounded-full shadow-sm transition-transform ${enabled ? 'translate-x-5' : 'translate-x-0.5'}`} />
    </button>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Manage your notifications, security, and preferences</p>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between py-1">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm font-medium text-gray-900">Email Notifications</p><p className="text-xs text-muted-foreground">Receive updates via email</p></div>
            </div>
          ...
          <div className="flex items-center justify-between py-1 border-t border-gray-50 pt-4">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm font-medium text-gray-900">SMS Notifications</p><p className="text-xs text-muted-foreground">Get text message alerts</p></div>
            </div>
        ...
        <button onClick={() => addToast('success', 'Preferences Saved', 'Your notification preferences have been updated.')}
          className="mt-5 w-full py-2.5 bg-cyan-600 text-foreground rounded-xl text-sm font-medium hover:bg-cyan-700 transition-colors">
      ...
      <div className="bg-card rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={currentPassword} onChange={e => setCurrentPassword(e.target.value)}
                className="w-full px-4 py-2.5 border border-border rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 pr-10" />
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-sm font-medium text-gray-900 flex items-center gap-2">
                  {twoFAEnabled ? <ShieldCheck className="w-4 h-4 text-green-600" /> : <ShieldOff className="w-4 h-4 text-muted-foreground" />}
                  Two-Factor Authentication
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">{twoFAEnabled ? `Active via ${twoFAMethod}` : 'Add an extra layer of security to your account'}</p>
              </div>
              {twoFALoading ? <Loader2 className="w-4 h-4 text-muted-foreground animate-spin" /> : twoFAEnabled ? (
            ...
            {!twoFAEnabled && !showOTPInput && (
              <div className="flex gap-2 mb-3">
                <button onClick={() => setTwoFAMethod('email')} className={`flex-1 py-2 text-xs font-medium rounded-lg border transition-colors ${twoFAMethod === 'email' ? 'bg-cyan-50 border-cyan-200 text-cyan-700' : 'bg-muted border-border text-muted-foreground'}`}>
                  <Mail className="w-3.5 h-3.5 inline mr-1.5" /> Email
                </button>
                <button onClick={() => setTwoFAMethod('sms')} className={`flex-1 py-2 text-xs font-medium rounded-lg border transition-colors ${twoFAMethod === 'sms' ? 'bg-cyan-50 border-cyan-200 text-cyan-700' : 'bg-muted border-border text-muted-foreground'}`}>
              ...
              <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-4 mt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleVerify2FA} disabled={otpCode.length !== 6 || verifying2FA}
                    className="px-4 py-2.5 bg-cyan-600 text-foreground rounded-xl text-sm font-medium hover:bg-cyan-700 disabled:bg-gray-300 disabled:cursor-not-allowed">
                    {verifying2FA ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Verify'}
                  </button>
                </div>
                <button onClick={() => { setShowOTPInput(false); setOtpCode(''); }} className="text-xs text-muted-foreground mt-2 hover:text-gray-700">Cancel</button>
              </div>
            )}
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 mt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleDisable2FA} disabled={!disablePassword || disabling2FA}
                    className="px-4 py-2.5 bg-red-600 text-foreground rounded-xl text-sm font-medium hover:bg-red-700 disabled:bg-gray-300">
                    {disabling2FA ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Confirm'}
                  </button>
                </div>
                <button onClick={() => { setShowDisable2FA(false); setDisablePassword(''); }} className="text-xs text-muted-foreground mt-2 hover:text-gray-700">Cancel</button>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-sm font-medium text-gray-900 flex items-center gap-2"><Monitor className="w-4 h-4 text-muted-foreground" /> Active Sessions</p>
                <p className="text-xs text-muted-foreground mt-0.5">Manage your logged-in devices</p>
              </div>
              <span className="text-xs font-medium text-muted-foreground bg-muted px-2.5 py-1 rounded-full">{sessions.length} active</span>
            </div>

            {sessionsLoading ? (
              <div className="py-4 flex justify-center"><Loader2 className="w-5 h-5 text-muted-foreground animate-spin" /></div>
            ) : sessions.length > 0 ? (
              <div className="space-y-2">
                {sessions.map((session) => {
                  const { device, browser } = parseDevice(session.userAgent);
                  return (
                    <div key={session.id} className={`flex items-center justify-between p-3 rounded-xl border ${session.isCurrent ? 'bg-green-50 border-green-200' : 'bg-muted border-border'}`}>
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${session.isCurrent ? 'bg-green-100' : 'bg-gray-200'}`}>
                          {device === 'Mobile' ? <Smartphone className="w-4 h-4 text-muted-foreground" /> : <Monitor className="w-4 h-4 text-muted-foreground" />}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-900 flex items-center gap-2">
                            {browser} on {device}
                          ...
                          <p className="text-xs text-muted-foreground">{session.ipAddress || 'Unknown IP'} &middot; {session.authMethod} &middot; Last active {formatDate(session.lastUsedAt)}</p>
                        </div>
                      </div>
                      {!session.isCurrent && (
              ...
              <p className="text-sm text-muted-foreground text-center py-3">No active sessions found</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-red-100 shadow-sm p-6">
        <h3 className="text-base font-semibold text-red-600 mb-4 flex items-center gap-2"><Trash2 className="w-4 h-4" /> Danger Zone</h3>
        <p className="text-sm text-muted-foreground mb-4">Request deletion of your account. This action is irreversible and must be approved by your property manager.</p>
