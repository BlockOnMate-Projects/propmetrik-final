...
function MessagesContent() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="bg-card rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ height: 'calc(100vh - 180px)' }}>
          ...
          <div className={`w-full md:w-80 lg:w-96 border-r border-gray-100 flex flex-col ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
            <div className="px-4 py-3 border-b border-gray-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowNewConvo(true)} className="p-1.5 text-muted-foreground hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors">
              ...
              <div className="relative">
                ...
                <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search conversations..."
                  className="w-full pl-9 pr-3 py-2 bg-muted border border-border rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              {loadingConvos ? (
                ...
                <button key={convo.id} onClick={() => handleSelectConvo(convo.id)}
                  className={`w-full px-4 py-3 flex items-start gap-3 text-left hover:bg-muted transition-colors border-b border-gray-50 ${activeConversation === convo.id ? 'bg-cyan-50/50' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {convo.lastMessageAt && <p className="text-[10px] text-muted-foreground flex-shrink-0 ml-2">{formatMessageTime(convo.lastMessageAt)}</p>}
                    </div>
                    <p className="text-xs text-muted-foreground truncate mt-0.5">{convo.lastMessage || 'No messages yet'}</p>
                  </div>
                  {(convo.unreadCount || 0) > 0 && (
                    <span className="w-5 h-5 bg-cyan-500 text-foreground rounded-full text-[10px] font-bold flex items-center justify-center flex-shrink-0">{convo.unreadCount}</span>
                  )}
                </button>
              )) : (
            ...
            {activeConvo ? (
              <>
                <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3">
                  <button onClick={() => setActiveConversation(null)} className="md:hidden p-1.5 text-muted-foreground hover:text-muted-foreground rounded-lg">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {loadingMessages ? (
                    ...
                    <div className="flex items-center justify-center py-12 text-sm text-muted-foreground">No messages yet. Say hello!</div>
                      ...
                      <div key={msg.id}>
                        ...
                        <div className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[80%] ${isMine ? 'order-2' : ''}`}>
                            <div className={`px-4 py-2.5 rounded-2xl ${isMine ? 'bg-cyan-600 text-foreground rounded-br-md' : 'bg-muted text-gray-900 rounded-bl-md'}`}>
                            ...
                            <div className={`flex items-center gap-1 mt-0.5 ${isMine ? 'justify-end' : 'justify-start'} px-1`}>
                              ...
                              {isMine && (msg.isRead ? <CheckCheck className="w-3 h-3 text-cyan-500" /> : <Check className="w-3 h-3 text-muted-foreground" />)}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="flex items-end gap-2">
                    <div className="flex-1 relative">
                      <textarea value={newMessage} onChange={e => setNewMessage(e.target.value)}
                        ...
                        className="w-full px-4 py-2.5 border border-border rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 resize-none"
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <button onClick={() => setShowNewConvo(true)} className="mt-4 px-4 py-2 bg-cyan-600 text-foreground rounded-xl text-sm font-medium hover:bg-cyan-700 transition-colors">
      ...
      {showNewConvo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/50" onClick={() => !creatingConvo && setShowNewConvo(false)}>
          <div className="bg-card rounded-2xl shadow-xl border border-border w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => !creatingConvo && setShowNewConvo(false)} className="p-1 text-muted-foreground hover:text-muted-foreground rounded"><X className="w-5 h-5" /></button>
            </div>
