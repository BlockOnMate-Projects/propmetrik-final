...
export default function EtlJobsPage() {
  ...
  const getStatusColor = (status: EtlJobStatus) => {
    switch (status) {
      case 'completed': return 'text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/20 border-green-500/30'
      case 'running': return 'text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/20 border-blue-500/30'
      case 'pending': return 'text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/20 border-yellow-500/30'
      case 'failed': return 'text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/20 border-red-500/30'
      case 'cancelled': return 'text-muted-foreground bg-muted/20 border-border/30'
        ...
        <div className="mb-6">
          <TerminalPanel title={`Running Jobs (${runningJobs.length})`} status="live">
            <div className="space-y-4">
              {runningJobs.map((job) => (
                <div key={job.id} className="p-4 bg-muted/30 border border-border">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <Loader2 className="w-4 h-4 text-blue-600 dark:text-blue-400 animate-spin" />
                      <div>
                        <div className="font-mono text-sm text-foreground">{job.job_type.replace('_', ' ').toUpperCase()}</div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-sm text-foreground">{job.progress || 0}%</span>
                      <button
                        onClick={() => cancelMutation.mutate({ id: job.id })}
                        className="px-3 py-1 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-500/30 font-mono text-[10px] hover:bg-red-900/50 transition-colors"
                      >
                        CANCEL
                      </button>
                    </div>
      ...
      <TerminalPanel title={`Job History (${jobs?.data?.length || 0})`}>
        {isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="h-20 bg-muted/30 animate-pulse" />
            ))}
          </div>
        ) : jobs?.data?.length === 0 ? (
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 font-mono text-[10px] text-muted-foreground">
                        ...
                        {(job.errors_count || 0) > 0 && (
                          <>
                            <span>•</span>
                            <span className="text-red-600 dark:text-red-400">{job.errors_count} errors</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {job.status === 'running' && (
                      <button
                        onClick={() => cancelMutation.mutate({ id: job.id })}
                        className="p-2 bg-muted hover:bg-zinc-700 border border-border hover:border-red-500 transition-colors"
                      >
