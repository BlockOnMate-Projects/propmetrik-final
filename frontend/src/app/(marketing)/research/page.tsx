                ...
                <div className="container mx-auto px-4 md:px-6">
                    {loading ? (
                        <div className="grid lg:grid-cols-2 gap-8">
                            {[...Array(2)].map((_, idx) => (
                                <div key={idx} className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 animate-pulse h-48" />
                            ))}
                        </div>
                    ) : reports.length === 0 ? (
                        <div className="text-center py-20 border border-zinc-800 rounded-lg bg-zinc-900">
                            <FileText className="w-12 h-12 text-zinc-600 mx-auto mb-4" />
                            <h3 className="text-xl font-bold text-white mb-2">Research coming soon</h3>
                                        ...
                                        {featured.map((report) => (
                                            <Link key={report.id} href={`/insights/${report.slug}`}>
                                                ...
                                                >
                                                    ...
                                                    <div className="p-6">
                                                        <span className="text-sm text-zinc-500 flex items-center gap-1 mb-3">
                                                        ...
                                                        {report.excerpt && <p className="text-zinc-400">{report.excerpt}</p>}
                                                    </div>
                                                </motion.div>
                                            </Link>
                                        ))}
                            ...
                            <div className="space-y-4">
                                {rest.map((report) => (
                                    <Link key={report.id} href={`/insights/${report.slug}`}>
                                        ...
                                        >
                                            <span className="text-sm text-zinc-500 flex items-center gap-1 mb-2">
                                                <Calendar className="w-4 h-4" />
                                                {formatDate(report.published_at)}
                                            </span>
                                            ...
                                            {report.excerpt && <p className="text-zinc-400 text-sm">{report.excerpt}</p>}
                                        </motion.div>
                                    </Link>
                                ))}
                            </div>
